...
def main():
    ...
    with ModelClient(args.url, "CustomDiffusionPipeline", init_timeout_s=args.init_timeout_s) as client:
        for req_idx in range(1, args.iterations + 1):
            print(f"Sending request ({req_idx}).")
            prompt_id = req_idx % len(prompts)
            prompt = prompts[prompt_id]
            prompt = np.array([prompt])
            prompt = np.char.encode(prompt, "utf-8")
            print(f"Prompt ({req_idx}): {prompt}")
            result_dict = client.infer_sample(requests=prompt)
            print(f"Result for for requests ({req_idx}).")

            for idx, image in enumerate(result_dict["result"]):
